...
faq_posts   { title, body, sourceLink, topicTag, language, isAutoGenerated, featured,
              unpublished, verdict, riskLevel, translations{ <lang>: {title, body} },
              clusterKey, clusterSize, clusterTotalSessions, clusterShare,
              sourceClaimIds[], createdAt }
...
| Health FAQs | `/faq` | Searchable, filterable by topic tag; each entry rendered in your interface language |
...
| FAQ CRUD (incl. the language each entry is written in), publish/unpublish, auto-generated badge | `/admin/faq` |
...
FAQ and trending entries follow the same model. Every `faq_posts` document records the
language it was written in — chosen by the admin on `/admin/faq`, or inherited from the
originating claim for auto-drafted posts. A reader on a different interface language gets the
entry translated in **one batched call per page**, cached on the post under
`translations.<lang>`, so the first visitor in a language pays once and everyone after them
reads for free. An entry already in the reader's language is served verbatim — which is why
the written-in language has to be recorded accurately rather than assumed to be English, and
why the client applies only the current language's cache (a stale translation from a
previously selected language must never outlive the switch).
